function createTrayController({
  ...
  function updateTray() {
    ...
    trayRef.setContextMenu(Menu.buildFromTemplate([
      ...
      { label: "显示控制器", click: showWindow },
      {
        label: "启动连接",
        enabled: !connected,
        click: () => Promise.resolve(start()).catch((error) => appendLog("host", `启动失败：${error.message}`)),
      },
      ...
      { type: "separator" },
      {
        ...
        click: () => {
          void Promise.resolve(requestQuit("tray-quit")).catch((error) => appendLog("host", `${error.code || "SHUTDOWN_FAILED"}：${error.message}`));
        },
      },
    ]));
  }
  ...
  return Object.freeze({ createTray, updateTray, dispose, dockIcon });
}
