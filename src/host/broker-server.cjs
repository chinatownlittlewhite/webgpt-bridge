...
function createHostBrokerServer({ app, hostSecurity, appendLog, endpoints, platform = process.platform, pid = process.pid, spawnSync }) {
  ...
  async function stop() {
    ...
    const activeSocket = brokerSocket;
    ...
    if (activeSocket && platform !== "win32") await fsp.rm(activeSocket, { force: true }).catch(() => {});
  }
  ...
  async function start(settings, runtime, { githubCliPath = "", proxyEnv = {}, brokerBootstrap } = {}) {
    ...
    brokerSocket = localBrokerSocketPath();
    if (platform !== "win32") await fsp.rm(brokerSocket, { force: true }).catch(() => {});
    ...
    brokerServer = net.createServer((socket) => attachConnection(socket, brokerBootstrap));
    await new Promise((resolve, reject) => {
      brokerServer.once("error", reject);
      brokerServer.listen(brokerSocket, () => {
        brokerServer.off("error", reject);
        resolve();
      });
    });
    if (platform !== "win32") await fsp.chmod(brokerSocket, 0o600);
    appendLog("local-broker", "已启动受控本机文件与终端代理。");
    return brokerServer;
  }

  return Object.freeze({
    createBootstrap: () => createBrokerBootstrap(),
    localBrokerSocketPath,
    socketPath,
    start,
    stop,
  });
