...
function createHostSettingsStore({
  app,
  safeStorage,
  bundledRuntimePath,
  ...
  if (typeof bundledRuntimePath !== "function") throw new TypeError("bundledRuntimePath is required");
  if (typeof spawnSync !== "function") throw new TypeError("spawnSync is required");

  const configPath = () => path.join(app.getPath("userData"), "settings.json");
  const secretPath = () => path.join(app.getPath("userData"), "runtime-key.bin");
  ...
  async function loadSettings() {
    try {
      ...
      return {
        ...settings,
        runtimePath: settings.runtimePath || bundledRuntimePath(),
  ...
  async function writeSettings(input) {
    const normalized = normalizeSettings(input, defaultSettings());
    const settings = {
      ...normalized,
      runtimePath: input.runtimePath || bundledRuntimePath(),
    };
