...
function createRuntimeHost({ settingsStore, startupPreflight, hostBroker, appendLog, resetLogs, spawnSync, endpoints, platform = process.platform, env = process.env }) {
  const { mcpHost, mcpPort } = endpoints;
  let serverProcess;
  let tunnelProcess;
  ...
  async function prepare() {
    ...
    const brokerBootstrap = hostBroker.createBootstrap();
  ...
  async function startAgent(preflight) {
    ...
    const baseEnv = {
      ...
      LPC_LOCAL_BROKER_SOCKET: hostBroker.socketPath(),
