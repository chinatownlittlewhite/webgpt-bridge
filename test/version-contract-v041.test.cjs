...
test("v0.4.5 desktop and v0.9.1 Agent versions are explicit and synchronized with runtime metadata", () => {
  ...
  assert.equal(desktop.version, "0.4.5");
  assert.equal(desktopLock.version, "0.4.5");
  assert.equal(desktopLock.packages[""].version, "0.4.5");
