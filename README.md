...
前三档中，首次批准某一普通权限类别后会在**本次连接**自动记住，同类后续请求不再重复询问；例如同一外部主机的网络访问、Git 远端写入、GitHub 写入、Docker、文件权限修改或相同 sandbox 扩权。工作区外的普通本机路径 read/list 需要显式 Host 授权，Desktop/Downloads/Documents 也保持首次显式授权；这些授权只在当前连接内按路径/目录与操作类型生效。敏感路径的 read/list 不会被记住，每次显式确认只签发一次、短时有效的单次授权；敏感写入/执行和系统路径始终不可授权，即使选择“完全控制”也一样。停止连接、重启应用或切换权限模式后连接内授权全部清空。“是否运行项目/执行项目脚本”不再作为权限请求。每个文件批次最多 20 项，并在实际写入前重新核对 SHA-256；任一文件发生变化，整个批次不会执行。
...
- 前三档对普通跨边界操作按权限类别确认，并在本次连接内记住允许复用的批准类别；敏感路径 read/list 始终使用显式单次授权，不进入连接级记忆。谨慎模式仍会确认删除/移动、本地 Git 变更等明显副作用操作。
