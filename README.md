...
| 完全控制（无确认） | WebGPT Bridge 不显示权限确认；App-owned 本机通道可访问敏感/系统路径、任意联网和远端写入，并可执行 shell、SSH、提权及绝对路径命令。只保留请求格式约束和操作系统自身权限限制。 |
...
在前三档中，本机代理默认保护 SSH 密钥、钥匙串/凭据、浏览器资料、云凭据目录、`.env` 与包管理器认证文件、WebGPT Bridge 自身密钥存储及系统目录；高风险访问需要按权限类别确认。完全控制会绕过这些 Bridge 级保护，因此只应在你明确希望模型直接控制本机时启用。macOS/Windows 自身的隐私、ACL 和系统安全限制仍然有效。
...
- “完全控制（无确认）”是显式高风险模式：桌面本机通道会绕过 Bridge 的敏感路径、系统路径、shell/SSH、提权、绝对可执行路径和确认策略；操作系统自身权限与隐私保护仍然生效。
